...
const core =
  ...
  'SIGNAL AT REST through your FINAL MESSAGE: BARE REST = an ordinary handoff (queues for the human); ```done = a checked completion card; ```awaiting = park ONLY a specific external-human (`human:`) or timestamp (`timer:`) gate, never CI/bots/releases/another session; ```question = the operator ask (a question IS the handback — no second fence). Automatable waits (CI/bot review/release/merge) stay ACTIVE via Monitor/background Bash, not awaiting. Load the `fray:worker` skill for the full contract.';
