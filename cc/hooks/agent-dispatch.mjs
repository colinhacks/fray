...
RUN LONG OPS TO COMPLETION INLINE — do NOT background-and-rest. A build, test run, install, CI watch, relink, or any long operation must be polled INLINE (read its output file / \`ps\` / the result) until it FINISHES; then complete the task and return your deliverable. Do NOT background it and rest on a waiter/monitor/"it will notify me" — that strands the task and forces a wasteful orchestrator round-trip to resume you (the SubagentStop guard will BLOCK such a rest). Come to rest ONLY with a real deliverable, or on a GENUINE external blocker you cannot poll (a human decision) — and then commit your WIP first and say so explicitly.
...
YOU OWN YOUR THREAD'S \`status:\` — SET IT YOURSELF to a VALID value; the orchestrator does NOT clean up after you. The ONLY allowed statuses are EXACTLY: \`planning · planned · active · blocked · done · dismissed\` (any other word — "ready"/"landing"/"investigated"/"root-caused"/"complete" — is INVALID and breaks the board validator; legacy \`todo\`/\`plan\`/\`enqueued\`/\`needs-decision\` are still ACCEPTED as read-aliases but write the canonical word). When your work is genuinely complete, set \`status: done\` (or \`dismissed\` if decided-not-to-pursue). If the thread CANNOT RUN right now, set \`status: blocked\` plus EXACTLY ONE resolution-mechanism field saying HOW it unblocks: (a) a HUMAN must decide/act → set NO machine field, add a one-line \`status_text:\` stating the decision/action needed (this is the ⚖ awaiting-you queue, surfaced YELLOW/prominent); (b) it waits on OTHER fray threads and/or an external gate → \`blocking_threads: [<slug>, …]\` — a bare slug is another fray thread (auto-fires when they all finish); a typed gate parks it (\`pr:owner/repo#123\` · \`issue:owner/repo#5\` · \`ci:<ref>\` · \`external:<free-form note>\`); legacy \`depends_on:\` is still read as an alias; (c) it should auto-re-poll on a timer → \`revalidate_at: <ISO-8601>\`. Set only ONE mechanism (>1 trips the validator). Also update the 1-line \`status_text:\` to the current truth. Do NOT leave the thread \`active\` when you've finished — that strands it. (The thread↔agent binding is AUTOMATIC — a hook records which agent serves which thread at dispatch; there is NO \`agents:\` frontmatter to maintain. Per-agent liveness is DERIVED from output freshness + the THREAD status, so just set the THREAD status correctly.)
