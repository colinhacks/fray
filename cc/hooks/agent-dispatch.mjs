...
YOU OWN YOUR THREAD'S \`status:\` — SET IT YOURSELF to a VALID value; the orchestrator does NOT clean up after you. The ONLY allowed statuses are EXACTLY: \`planning · planned · enqueued · active · needs-decision · blocked · done · dismissed\` (any other word — "ready"/"landing"/"investigated"/"root-caused"/"complete" — is INVALID and breaks the board validator; legacy \`todo\`/\`plan\` are still ACCEPTED as read-aliases for \`planned\`, but write the canonical word). When your work is genuinely complete, set \`status: done\` (or \`dismissed\` if decided-not-to-pursue); if it now needs a HUMAN decision/call, \`needs-decision\` (+ a one-line \`status_text:\` stating the decision needed — this is the ⚖ awaiting-you queue); if it is waiting on a NON-human thing — another in-flight fray thread → \`enqueued\` + \`depends_on:\`; an external PR/CI/merge → \`blocked\` (or \`enqueued\` + \`revalidate_at:\` to auto-re-poll). Also update the 1-line \`status_text:\` to the current truth. Do NOT leave the thread \`active\` when you've finished — that strands it. (The thread↔agent binding is AUTOMATIC — a hook records which agent serves which thread at dispatch; there is NO \`agents:\` frontmatter to maintain. Per-agent liveness is DERIVED from output freshness + the THREAD status, so just set the THREAD status correctly.)
