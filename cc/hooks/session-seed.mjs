...
- The dispatched sub-agent EDITS ITS OWN thread .md in place (+ a .fray/<thread>.findings/<id>.md sidecar only for parallel fan-out); the orchestrator does cross-thread linkage/reversals, the decision queue, dispatch/synthesis, and config.yml — never re-transcribing what the agent wrote. The agentId→thread binding is AUTOMATIC (a hook records it at dispatch); there is no agents:[] frontmatter to maintain.
  ...
  if (!agentTeamsEnabled()) {
    ...
    if (!alreadyNotified) {
      teamsNotice =
        ...
        'fray\'s core steering model — ALWAYS STEER, don\'t re-dispatch: SendMessage to a RUNNING or COMPLETED sub-agent, warm-resume — DEPENDS on it; without it fray silently degrades to cold re-dispatch (you lose live steering + warm-resume and fall back to the `blocked` + `blocking_threads` sequencing model). ' +
