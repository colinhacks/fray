...
Thread statuses are `planning`, `planned`, `active`, `blocked`, `done`, and `dismissed`. Legacy `todo`, `plan`, `deferred`, `enqueued`, and `needs-decision` validate as read aliases, but write the canonical set going forward. Use `planning` when the active deliverable is the design itself, `planned` for scoped/parked work that is not currently being worked, `active` only for live work with an executing child or immediate next action, and `blocked` when work cannot run right now (human decision, dependency, or external wait described in the thread body). After every child result or verification result, set the status from current ground truth; never leave `active` as stale intent after the work has resolved.

**Board hygiene is the orchestrator's standing job.** Every turn the board surfacing is a STANDING PROMPT to notice rot, not just a dispatch menu. Proactively run a cleanup pass whenever human-blocked work is growing or many threads look stale — do NOT wait to be told. Human-blocked work should nearly always be near-empty: it means live work blocked on a SINGLE human judgment call RIGHT NOW. A planning/design thread whose questions are actively being worked is `planning`; scoped but parked work is `planned`. A completed investigation whose deliverable was the findings is `done`. Superseded/abandoned is `dismissed`. When surfacing decisions to the human, surface only the genuine current-workstream calls — never dump the full board.
...
A dispatched child is itself a LEAD (L1) that can dispatch its own children (L2s). Two uses. **Use A — fan out L2s for independent sub-work, then synthesize:** when an L1's task decomposes into genuinely-independent pieces, spin up as many (or as few) L2s as the task genuinely warrants, collect, and synthesize ONE deliverable rather than grinding serially. The count is pure JUDGMENT driven by the work, NOT a fixed band — one L2 for a clean isolated context, two for a simple split, many for a wide fan-out. An L2 earns its dispatch for EITHER parallelism (the wall-clock win) OR context isolation (a focused subtask deserves its own fresh context window even with no parallelism — a single L2 just for a clean context is legitimate and encouraged). E.g. research "to its logical conclusion" with one L2 per architecture path; a survey with one L2 per attack/reference/file. Dispatch when pieces are INDEPENDENT and either parallelism saves wall-clock or a subtask deserves a clean context; stay inline for a real dependency chain or trivial work. Mechanics: as many as the task warrants but NEVER an unbounded swarm (real ceiling is 429 backoff, not a fixed count or one-at-a-time); an L2 returns to the DISPATCHING L1; do NOT set `name`/`team_name` (the dispatch hook auto-strips both — they strand nested agents); TIER each L2 by load (survey → cheaper model, hard analysis/engineering → top model); the L1 COLLECTS + SYNTHESIZES into one deliverable (never scattered sub-results) and still owns its thread + reports up. The budget governor forbids UNBOUNDED fan-out (a runaway swarm), not bounded parallelism. **Use B — the implementer self-organizes its own review via L2s:** for substantive new functionality, the loop is PLAN → dispatch a level-2 plan-review → second-pass the plan → IMPLEMENT → dispatch level-2 self-review (multiple PARALLEL lenses for a major change: correctness/security/subsystem) → CRITICALLY INCORPORATE (the implementer judges reviews on merit and folds only valid findings — it does NOT blind-trust; level-2 reviewers have narrower/staler context). Invariants: review at BOTH plan and implementation stages; depth scales with blast radius (trivial → no nesting; major → parallel-lens panel); reviews are advice, not verdicts. Both uses compose with, not replace, the orchestrator's own independent review pass on the returned work. **L1 owns its full loop — orchestrator must NOT babysit the review-fix cycle.** An L1 dispatched to BUILD or IMPLEMENT OWNS its full loop (build → adversarial self-review via its own L2 → fix → re-review → until clean) and reports UP only when (a) DONE or (b) BLOCKED on a human-owned decision. The orchestrator must NOT run a sibling reviewer and relay findings to the L1 turn-by-turn — that scatters one effort's review-fix cycle across the orchestrator's context. The tell: repeatedly SendMessage-ing the same L1 with "reviewer found X, fix it" instead of the L1 having self-reviewed and surfacing only human-owned decisions. What routes up: architecture, default/security/brand/API-config calls, scope changes. Independence is preserved because the L1's self-review L2 is a fresh-context adversarial agent. (Exception: a final pre-merge gate review by the orchestrator is fine; mediating the iterative build-fix cycle is not.)
