...
**Board hygiene is the orchestrator's standing job.** Every turn the board surfacing is a STANDING PROMPT to notice rot, not just a dispatch menu. Proactively run a cleanup pass whenever `needs-decision` is growing or many threads look stale — do NOT wait to be told. `needs-decision` should nearly always be near-empty: it means live work blocked on a SINGLE human judgment call RIGHT NOW. A planning or design thread with open questions is `todo` (questions belong in the thread body, not as a board-level status). A completed investigation whose deliverable was the findings is `done`. Superseded/abandoned is `dismissed`. When surfacing decisions to the human, surface only the genuine current-workstream calls — never dump the full board.
