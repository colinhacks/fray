...
Priority order: answer direct user questions immediately and concisely; otherwise treat unhandled child completions as a strict inbox. Before unrelated work or any new dispatch, drain the oldest unhandled result with `fray_next`/`fray_reconcile` unless the user explicitly asks a higher-priority direct question; after answering that interruption, resume the inbox immediately. Completed child results are not optional notifications and not only decision prompts: reconcile the oldest result, update the thread/backlog, report it in chat, mark handled only after reporting, then check for the next result.
...
Every child prompt must require an orchestration-ready final assistant response. That final response is embedded in native completion follow-ups and remains the primary report consumed by `fray_next`/`fray_reconcile`; sidecars are only for long raw artifacts. The final output must include: verdict/status; what was done; changed files, artifacts, clone path, and commit SHA when applicable; verification commands and results; blockers, caveats, and remaining risks; and one concrete next action. Empty or missing final output is an incomplete handoff/bug, not normal success: Fray first recovers from the child `sessionFile`, then marks the handoff incomplete/needs-retry if no final text exists. No silent completion, progress-only final state, or "done" without usable result data.
...
A completed child is not handled until the orchestrator reconciles it, reports it in chat, and marks it handled. Native Pi completion follow-ups embed the oldest unhandled child's run metadata and captured final output directly in the prompt; `fray_next` remains the strict inbox and durable source of truth, and `fray_reconcile` remains the mark-handled path. Reconciliation reviews and completes the thread state; it no longer implies the orchestrator manually performs every thread-doc update. Do not batch-reconcile silently. Handle one child at a time: use the embedded final output, review/complete the thread/backlog synthesis, report in chat, mark handled, call `fray_next`, repeat.
...
3. If the embedded output says the handoff is incomplete or has no final text, treat the result as incomplete/needs-retry; use `fray_next`/`fray_reconcile`, the findings path, or the child session file only as fallback evidence.
...
7. Only after the chat report, call `fray_reconcile` with `markHandled: true` (or legacy `markReconciled: true`).
8. Call `fray_next` for the next unhandled child result and repeat before unrelated work or any new dispatch. If a direct user question interrupts, answer it and resume the inbox immediately.
9. When `fray_next` is clear, continue with recorded follow-ups, dispatch, or steering.
...
- Do not drop child completions; treat them as a strict inbox, reconcile/report one at a time, and drain the oldest before unrelated work or any new dispatch.
