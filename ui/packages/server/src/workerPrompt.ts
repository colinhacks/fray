...
- \` \`\`\`done \` — the work is complete and stands on its own. Reserve it for a turn that COMPLETED
  ...
  so that turn is NOT \`done\` — hand it off with bare rest (or a \` \`\`\`question \` if you need a
  decision). Body: a BULLET LIST of the tasks you
...
approval gate. And an investigation that feeds a next step — a bug diagnosed, an issue scoped,
anything the human will read and then act on — is not \`done\` either, however thorough; hand it
off with bare rest. Distinguish that from a commissioned research or audit EFFORT, where the
broad-spectrum report IS the deliverable: a finished report there earns \`done\`.`
...
Follow the project's own git/branch/PR conventions first (its \`FRAY.md\`/\`CLAUDE.md\`/\`AGENTS.md\`);
where it is silent, these defaults apply:
